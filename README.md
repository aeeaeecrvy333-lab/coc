...
共计 **286+ 页面**，包含 69 个概念页面、211 个实体页面、6 个来源摘要。
...
除知识库外，本项目还包含三款面向玩家的工具，均基于纯前端技术构建（HTML/CSS/JS，无框架依赖），可直接在浏览器中打开使用。
...
- **响应式设计** — 适配桌面和移动端
- **E2E 测试** — 已移除（原有 Playwright 测试和 Python 备选脚本已删除）

### 向火独行 电子陪跑

> `apps/alone-against-the-flames-app/` — 单人模组《向火独行》电子陪跑工具

一个完整的 CoC 7e 单人模组《向火独行（Alone Against the Flames）》电子化游玩工具，270 个条目全文结构化，引擎驱动剧情流转与规则解算。

#### 核心功能

| 功能 | 说明 |
|------|------|
| 剧情引擎 | 270 条目节点式流转，支持多分支路径 |
| 检定系统 | 技能/属性/派生值检定，自动识别难度（困难/极难）和模式（奖励骰/惩罚骰） |
| 战斗系统 | 5 个战斗场景，对抗检定 + 多回合近战，弹出层 UI |
| 效果系统 | HP/SAN/MP/Luck 调整、技能成长、物品管理、检定门控效果 |
| 3D 骰子 | DiceBox 物理引擎，百分骰（2×d10）、奖惩骰支持 |
| 角色导入 | .coc7 文件导入，与角色卡创建器互通 |
| 进度保存 | localStorage 自动存档，刷新不丢失 |
| 结局复盘 | 18 个结局，含路径回顾、状态快照、里程碑、技能成长 |

#### 架构

```
engine/     ← 纯逻辑层（模组引擎、对抗检定、战斗状态机）
adapters/   ← 外部接口适配（角色卡、骰子库）
data/       ← 声明式数据（模组条目、战斗脚本、技能数据）
ui/         ← 渲染层（故事面板、角色面板、战斗弹出层）
```
...
在创建器中完成角色创建后，导出 `.coc7` 文件，可在追踪器或向火独行中导入使用。三个应用共享一致的暗色金色视觉主题和游戏数据体系。
...
│   └── alone-against-the-flames-app/  # 向火独行电子陪跑
│       ├── index.html           # HTML 入口
│       ├── css/                 # 样式（app.css + combat.css）
│       ├── js/
│       │   ├── engine/          # 引擎层（模组引擎/对抗检定/战斗状态机）
│       │   ├── adapters/        # 适配层（角色卡/骰子）
│       │   ├── data/            # 数据层（模组条目/战斗脚本/技能）
│       │   ├── ui/              # 渲染层（故事/角色面板/战斗弹出层）
│       │   └── app.js           # 主入口
│       ├── assets/figures/      # 模组插图
│       └── scripts/             # 工具脚本（全文解析器）
└── README.md
...
open apps/alone-against-the-flames-app/index.html  # 向火独行
```
